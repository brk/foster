...
struct FosterGCLowering : public llvm::FunctionPass {
  static char ID;
  llvm::Value* write_barrier_fn;
  bool no_barriers;
      ...
      for (BasicBlock::iterator II = BB->begin(), E = BB->end(); II != E;) {
        if (IntrinsicInst *CI = dyn_cast<IntrinsicInst>(II++)) {
          ...
          case Intrinsic::gcwrite:
            if (no_barriers) {
              lower_gcwrite_to_plain_store(CI);
            } else {
              lower_gcwrite(CI);
            }
            break;
  ...
  void lower_gcwrite_to_plain_store(IntrinsicInst* CI) {
...
char FosterGCLowering::ID = 0;

} // unnamed namespace

namespace llvm {
  void initializeFosterGCLoweringPass(llvm::PassRegistry&);
}

INITIALIZE_PASS(FosterGCLowering, "foster-gc-lowering",
                "Foster GC intrinsics lowering pass",
                false, false)

namespace foster {

Pass* createFosterGCLoweringPass(bool nb) { return new FosterGCLowering(nb); }
