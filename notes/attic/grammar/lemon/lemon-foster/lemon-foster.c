...
PRIVATE int translate_code(struct lemon *lemp, struct rule *rp){
  ...
  char zLhsGet[50];      /* Convert the LHS symbol into this string */
  char zLhsSet[50];      /* Convert the LHS symbol into this string */
  ...
  lhsdirect = 0;
  if( lhsdirect ){
    sprintf(zLhsSet, "yymsp[%d].minor Yy%d",1-rp->nrhs,rp->lhs->dtnum);
    sprintf(zLhsGet, "yymsp[%d].minor unYy%d",1-rp->nrhs,rp->lhs->dtnum);
  }else{
    rc = 1;
    //sprintf(zLhsSet, "yylhsminor /*Yy%d*/",rp->lhs->dtnum);
    //sprintf(zLhsGet, "yylhsminor /*unYy%d*/",rp->lhs->dtnum);
  }
    ...
    if( ISALPHA(*cp) && (cp==rp->code || (!ISALNUM(cp[-1]) && cp[-1]!='_')) ){
      ...
      *xp = 0;
      if( rp->lhsalias && strcmp(cp,rp->lhsalias)==0 ){
        append_str("yylhsminor", 0,0,0);
        /*
        if (seenEq) {
          append_str(zLhsGet,0,0,0);
        } else {
          append_str(zLhsSet,0,0,0);
        }
        */
        cp = xp;
        lhsused = 1;
          ...
          if( rp->rhsalias[i] && strcmp(cp,rp->rhsalias[i])==0 ){
            if( i==0 && dontUseRhs0 ){
              ...
              ** the token number of X, not the value of X */
              ...
              append_str("(unYy%d (yypParser `getMinor` %d))",0,dtnum,i-rp->nrhs+1);
            }
            cp = xp;
            used[i] = 1;
            break;
  ...
  ** saved LHS value now. */
  if( lhsdirect==0 ){
    //append_str("Yy%d (", 0, rp->lhs->dtnum, 0);
    //append_str(zLhsGet, 0, 0, 0);
    //append_str(") >^ yymspMinor[%d]", 0, 1-rp->nrhs, 0);
    append_str("setMinor yylhsminor^ yypParser %d;\n", 0, 1-rp->nrhs, 0);
  }
...
void print_stack_union(
  ...
  */
  ...
  fprintf(out,"type case YYMINORTYPE\n"); lineno++;
  //fprintf(out,"  int yyinit;\n"); lineno++;
  //
  //fprintf(out,"   of $Yy0 %sTOKENTYPE\n",name); lineno++;
  fprintf(out,"   of $Yy0 Token\n"); lineno++; // hardcode ParseTOKENTYPE
  //
...
void ReportTable(
  ...
  */
  ...
  fprintf(out,"yy_action :: Array Int32;\n"); lineno++;
  fprintf(out,"yy_action = prim mach-array-literal\n"); lineno++;
  for(i=j=0; i<n; i++){
  ...
  fprintf(out, "yy_lookahead :: Array Int8;\n"); lineno++;
  fprintf(out,"yy_lookahead = prim mach-array-literal\n"); lineno++;
  for(i=j=0; i<n; i++){
  ...
  fprintf(out, "yy_shift_ofst :: Array Int32;\n"); lineno++;
       //minimum_size_type(mnTknOfst, lemp->nterminal+lemp->nactiontab, &sz));
  fprintf(out, "yy_shift_ofst = prim mach-array-literal \n"); lineno++;
  lemp->tablesize += n*sz;
  for(i=j=0; i<n; i++){
    int ofst;
    stp = lemp->sorted[i];
  ...
  fprintf(out, "yy_reduce_ofst :: Array Int32;\n"); lineno++;
  fprintf(out, "yy_reduce_ofst = prim mach-array-literal\n"); lineno++;
  ...
  fprintf(out, "yy_default :: Array Int32;\n"); lineno++;
  fprintf(out, "yy_default = prim mach-array-literal \n"); lineno++;
  n = lemp->nxstate;
  lemp->tablesize += n*szActionType;
  for(i=j=0; i<n; i++){
    stp = lemp->sorted[i];
  ...
  */
  fprintf(out,"yyRuleInfoNRhs :: Array Int32;\n"); lineno++;
  fprintf(out,"yyRuleInfoNRhs = prim mach-array-literal\n"); lineno++;
  for(i=0, rp=lemp->rule; rp; rp=rp->next, i++){
    fprintf(out,"  %4d      /* (%d) ",-rp->nrhs,i);
    rule_print(out, rp);
    fprintf(out," */\n"); lineno++;
  }
  fprintf(out,";\n"); lineno++;
  fprintf(out,"yyRuleInfoLhs :: Array Int8;\n"); lineno++;
  fprintf(out,"yyRuleInfoLhs = prim mach-array-literal\n"); lineno++;
  for(i=0, rp=lemp->rule; rp; rp=rp->next, i++){
  ...
  //if( i ){ fprintf(out,"        YYMINORTYPE yylhsminor;\n"); lineno++; }
  ...
  for(rp=lemp->rule; rp; rp=rp->next){
    ...
    fprintf(out,"      of %d /* ", rp->iRule);
    writeRuleText(out, rp);
    fprintf(out, " */\n"); lineno++;
    for(rp2=rp->next; rp2; rp2=rp2->next){
      if( rp2->code==rp->code && rp2->codePrefix==rp->codePrefix
             && rp2->codeSuffix==rp->codeSuffix ){
        fprintf(out,"      or %d /*! ", rp2->iRule);
