...
function format_board_oneline(board)
    gboard = {}
    for i = 1, #board do
        if board[i] == 0 then gboard[i] = '.'
        elseif board[i] == 1 then gboard[i] = 'x'
    ...
    return string.format("%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s", unpack(gboard))
end
...
if #arg == 1 and arg[1] == "bench" then -- decide wheter to play or to benchmark
    ...
    local t1 = os.clock()
    print ("Evaluating an empty board to depth " .. negaMax.maxdepth .. " took " .. (t1-t0) .. " secs.")
else
    --negaMax.maxdepth = 5
