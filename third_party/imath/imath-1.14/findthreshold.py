...
##
## This tool computes some timing statistics to help you select a
## suitable recursive multiplication breakpoint.  It uses the imtimer
## tool to run a series of tests varying precision and breakpoint, and
## prints out a summary of the "best" values for each category.
## Each summary line contains the following fields, tab-separated:
##
...
##
## You are responsible for reading and interpreting the resulting
## table to obtain a useful value for your workload.  Change the value
## of MP_MULT_THRESH in imath.h once you have a satisfactory result.
##
...
def get_timing_stats(num_tests, precision, threshold, seed = None):
    ...
    Returns a tuple of (seed, bits time) where seed is the random seed
    used, bits is the number of bits per ooperand, and time is a float
    giving the total time taken for the test run.
    """
    if seed is None:
        seed = int(time.time())
